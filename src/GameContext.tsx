...
type GameContextValue = {
  ...
  /** Jumps straight to a results screen for one ending, for reading the
   *  general's sign-off without flying five minutes first. Development
   *  builds only - see previewEnding. */
...
export function GameProvider({ children }: { children: ReactNode }) {
   ...
   *
   * Three endings each need the whole five minutes flown a particular way to
   * be seen once, which makes proof-reading the general's sign-off (or the
   * titles, or the layout at that length of text) a twenty-minute job per
   * edit. This is the shortcut, and it is fenced to development builds:
   * `import.meta.env.DEV` is a compile-time constant, so in a production
   * bundle the body is dead code and the lobby button that calls it is not
   * rendered at all.
   */
  ...
  const value = useMemo<GameContextValue>(() => ({ runtime, snapshot, readInput, advance, start, restart, unlockTutorialControl, dismissMissionDebrief, previewEnding, setMobileInput, quality, setQuality, language, setLanguage, t: STRINGS[language] }), [advance, dismissMissionDebrief, previewEnding, quality, readInput, restart, unlockTutorialControl, setMobileInput, setQuality, snapshot, start, language, setLanguage])
