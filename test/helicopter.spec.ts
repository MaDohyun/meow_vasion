...
describe('helicopter ambush behaviour', () => {
  it('keeps the speed ladder honest: a moving craft outruns it, a parked one does not', () => {
    // Halved by design: the chase now sits below cruise, so the helicopter is
    // a zone threat rather than a pursuer - it punishes hovering, beam work
    // and flying into its patch, and any craft under way simply leaves.
    expect(HELICOPTER_CHASE_SPEED).toBeLessThan(DRONE_DEFAULTS.maxSpeed)
    // Still a real closing speed against a craft that is not flying away.
    expect(HELICOPTER_CHASE_SPEED).toBeGreaterThan(HELICOPTER_ROAM_SPEED * 3)
  ...
  it('locks on when the player crosses the detection range and runs them down', () => {
    ...
    // Fifty metres at the halved chase speed is still under three seconds:
    // a craft that sits there gets rammed, on a timer it can read.
    expect(ticks).toBeLessThan(60 * 3)
  })
  ...
  it('never catches a craft that flees at cruise - the ram punishes staying, not flying', () => {
    ...
    // Not merely uncaught: the gap opens, so simply cruising away is a real
    // escape rather than a stalemate.
    const gap = Math.hypot(enemy.position.x - player.x, enemy.position.z - player.z)
    expect(gap).toBeGreaterThan(openingGap)
  })
