...
describe("earth's last resort", () => {
  ...
  it('shows both of its aimed shots before either leaves', () => {
    ...
    for (const length of telegraphs) expect([BATTLESHIP_MAIN_GUN_TELEGRAPH, ANTI_AIR_TELEGRAPH]).toContain(length)
    expect(telegraphs).toContain(BATTLESHIP_MAIN_GUN_TELEGRAPH)
    expect(telegraphs).toContain(ANTI_AIR_TELEGRAPH)
  })

  it('borrows the anti-air lock and answers it two barrels at a time', () => {
    const { state, player, ship } = launch()
    for (const enemy of state.slots) if (enemy !== ship) enemy.active = false
    const shells = () => state.projectiles.filter((projectile) => projectile.active && projectile.kind === 'missile')
    ...
    expect(ship.telegraphLength).toBeCloseTo(ANTI_AIR_TELEGRAPH, 5)
